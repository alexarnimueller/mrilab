...
class MultiBrain(object):
    ...
    def vox_brain(self, scaletuple=(4, 4, 4), plot=True):
        ...
        if len(self.img.shape) == 4:
            tup = tuple(list(scaletuple) + [1])
        else:
            tup = scaletuple
        self.data = downscale_local_mean(self.img.get_data(), tup)
...
if __name__ == "__main__":
    ...
    # rescale to 0, 255
    print "Rescaling to 256 bins..."
    train.rescale(nbins=255, numproc=12, overwrite=True)
    ...
    print "Binarizing > 200..."
    train.data = np.array(train.data > 200., dtype='float')
    
    #bins = 200
    #print("Compute histogram with %i bins..." % bins)
    #train.data = np.array([np.histogram(x, bins)[0] for x in train.data], dtype='float')
    #test.data = np.array([np.histogram(x, bins)[0] for x in test.data], dtype='float')
    ...
    #mi = mutual_info_classif(train.data, train.targets)
    #train.mask = (mi > (np.mean(mi) + 2 * np.std(mi)))
    ...
    pipeline = Pipeline([# ('scl', StandardScaler()),
                         # ('clf', RandomForestClassifier(n_estimators=500, class_weight={0: 10}, n_jobs=-1))])
                         # ('clf', AdaBoostClassifier(n_estimators=500))])
                         ('clf', SVC(C=0.1, kernel='rbf', probability=True, class_weight={0: 10}))])
    
    cv = 5
    print("%i-fold cross-validation" % cv)
    scores = cross_val_score(pipeline, train.data, train.targets, cv=cv)
    print("Average log loss: %.3f±%.3f" % (np.mean(scores), np.std(scores)))
